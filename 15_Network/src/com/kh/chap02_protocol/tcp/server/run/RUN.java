...
public class RUN {
	 ...
	 * -서버와 클라이언트 모두 연결이 되어 있어야 데이터 교환한다.
	 * -신뢰성 있고 안전한 데이터 전달이 가능
	 * -클라이언트 서버간의 연결이 되어 있으므로 실시간으로 데이터 송수신이 가능 
	 * 
	 * Socket 
	 * -클라이언트와 서버간에 실시간ㅁ "통신"에 사용되는 객체
	 * -소켓이 소유한 ip번호와 port번호를 통해 누구의 소켓인지 식별할 수 있다.
	 * -입/출력을 위한 input/output스트림을 소유하고 있다. 
	 * 
	 * ServerSocket
	 * -서버에서 생성하는 소켓으로 클라이언트의 요청을 대기한다,
	 * -연결요청이 들어오면  Socket을 생성하여 클라이언트에게 전달한다.
	 * 
	 * ClaientSocket(socket)
	 * -서버와 통신에 사용되는 클라이언트 소켓
	 * -서버의 ip주소, 포트번호를 통해 서버에세 연결요청한다. 
	 */
	public static void main(String[] args) {
		//1) 서버 소켓의 포트번호 지정
		//	-0~1023 : 시스템 포트번호로 사용 불가
		//	-1024~65535 요 사이의 포트번호 사용 
		int port =30000;
		
		//2) 섭 소켓 객체 생성
		//-현재 pc의 ip주소와 포트번호를 통해 소켓 생성
		...
		try {
			...
			while(true) {
				...
				//클라이언트의 요청이 들어오기 전까지 대기 후, 요청아 들어오면
				...
				//4)연결된 클라이언트와 통신할수 있는 입출력 스크림 생성 
