...
/*
 * 직렬화가 되지 않은 일반 클래스는 입출력이 불가능ㅎ나다. 
 * 
 * 직렬화(Serialization)
 * -객체의 상ㅊ태를 그대로 저장하거나, 복원하ㅣ는 방식을 직렬화/역직렬화라고 부른다
 *  객체 데이터를 byte로 변환하여 입/출력 매체에 저장하는 과정을 직렬화
 *  -저장된 바이트를 다시 객체상채로 복원하는 과정을 역직렬화 
...
public class Phone implements Serializable {
	private String name;
	private int price;
	//transient : 객체 입출력시 해당값을 입출력하지 않게 하는 예약어 transient
